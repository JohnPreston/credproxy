...
.. meta::
    :description: Complete Docker Compose deployment guide for CredProxy including basic setup, socat proxy sidecar, and production configurations
    :keywords: CredProxy, Docker Compose, deployment, socat proxy, ECS metadata, testing

This guide provides comprehensive Docker Compose deployment options for CredProxy, from simple local development to testing configurations with socat proxy sidecar.

Quick Start
-----------

For most users, the basic Docker Compose setup is the recommended starting point:
...
This will start CredProxy with default configuration on port 1338.

Basic Setup
-----------

The basic Docker Compose setup provides a simple CredProxy instance for local development and testing.
...
.. code-block:: bash
    ...
    CREDPROXY_METRICS_PROMETHEUS_PORT=9090
    CREDPROXY_DYNAMIC_SERVICES_ENABLED=false

Socat Proxy Sidecar for ECS Metadata
------------------------------------

The socat proxy sidecar enables ECS metadata-style credential forwarding, allowing applications to use the same credential provider interface as in ECS containers.
...
For comprehensive multi-SDK testing with CloudFormation, see :doc:`cloudformation` for the complete quickstart guide that helps you create IAM roles and test across multiple AWS SDKs.
...
For testing scenarios requiring external metrics access:
...
.. code-block:: bash
    ...
    # Remove generated files (for multi-SDK setup)
...
- :doc:`socat-proxy` - Detailed socat proxy configuration guide
